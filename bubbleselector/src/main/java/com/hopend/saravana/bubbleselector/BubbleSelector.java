...
package com.hopend.saravana.bubbleselector;
...
/**
 * The bubbleselector is a single or double-sided version of a {@link android.widget.SeekBar}
 * with discrete values. Whereas the thumb for the SeekBar can be dragged to any
 * position in the bar, the bubbleselector only allows its thumbs to be dragged to
 * discrete positions (denoted by tick marks) in the bar. When released, a
 * bubbleselector thumb will snap to the nearest tick mark.
 * This version is forked from edmodo range bar
 * https://github.com/edmodo/range-bar.git
 * <p>
 * Clients of the bubbleselector can attach a

 * have
 * been moved.
 */
public class BubbleSelector extends View {

    // Member Variables ////////////////////////////////////////////////////////

    private static final String TAG = "bubbleselector";
   ...
   // private PinView mRightThumb;
    ...
    public BubbleSelector(Context context, AttributeSet attrs) {
        super(context, attrs);
        rangeBarInit(context, attrs);
    }

    public BubbleSelector(Context context, AttributeSet attrs, int defStyle) {
      ...
      //  mRightThumb.init(ctx, yPos, 0, mPinColor, mTextColor, mCircleSize, mCircleColor);
        ...
        if (mIsRangeBar) {
           ...
           // mLeftThumb.setXValue(getPinValue(mLeftIndex));
            if(Integer.valueOf(getPinValue(mLeftIndex)) == 1)
            {
                mLeftThumb.setXValue("Low");
            }
            else if((Integer.valueOf(getPinValue(mLeftIndex)) == 2))
            {
                mLeftThumb.setXValue("Medium");
            }
            else if((Integer.valueOf(getPinValue(mLeftIndex)) == 3))
            {
                mLeftThumb.setXValue("High");
            }
            pressPin(mLeftThumb);
        }
       ...
       /* if(Integer.valueOf(getPinValue(mLeftIndex)) == 1)
        {
            mRightThumb.setXValue("Low");
        }
        else if((Integer.valueOf(getPinValue(mLeftIndex)) == 2))
        {
            mRightThumb.setXValue("Medium");
        }
        else if((Integer.valueOf(getPinValue(mLeftIndex)) == 3))
        {
            mRightThumb.setXValue("High");
        }*/
    ...
    public void setSeekIntex(int index)
    {
        //mFirstSetTickCount = true;
        mLeftIndex = index+1;
    ...
    private void rangeBarInit(Context context, AttributeSet attrs) {
        ...
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.BubbleSelector, 0, 0);
        ...
        try {
            ...
            final float tickStart = ta
                    .getFloat(R.styleable.BubbleSelector_tickStart, DEFAULT_TICK_START);
            final float tickEnd = ta
                    .getFloat(R.styleable.BubbleSelector_tickEnd, DEFAULT_TICK_END);
            final float tickInterval = ta
                    .getFloat(R.styleable.BubbleSelector_tickInterval, DEFAULT_TICK_INTERVAL);
            ...
            mTickHeightDP = ta
                    .getDimension(R.styleable.BubbleSelector_tickHeight, DEFAULT_TICK_HEIGHT_DP);
            mBarWeight = ta.getDimension(R.styleable.BubbleSelector_barWeight, DEFAULT_BAR_WEIGHT_PX);
            mBarColor = ta.getColor(R.styleable.BubbleSelector_barColor, DEFAULT_BAR_COLOR);
            mTextColor = ta.getColor(R.styleable.BubbleSelector_textnewColor, DEFAULT_TEXT_COLOR);
            mPinColor = ta.getColor(R.styleable.BubbleSelector_pinColor, DEFAULT_PIN_COLOR);
            mActiveBarColor = mBarColor;
            mCircleSize = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    ta.getDimension(R.styleable.BubbleSelector_selectorSize, DEFAULT_CIRCLE_SIZE_DP),
                    getResources().getDisplayMetrics());
            mCircleColor = ta.getColor(R.styleable.BubbleSelector_selectorColor,
                    DEFAULT_CONNECTING_LINE_COLOR);
            mActiveCircleColor = mCircleColor;
            mTickColor = ta.getColor(R.styleable.BubbleSelector_tickColor, DEFAULT_TICK_COLOR);
            mActiveTickColor = mTickColor;
            mConnectingLineWeight = ta.getDimension(R.styleable.BubbleSelector_connectingLineWeight,
                    DEFAULT_CONNECTING_LINE_WEIGHT_PX);
            mConnectingLineColor = ta.getColor(R.styleable.BubbleSelector_connectingLineColor,
                    DEFAULT_CONNECTING_LINE_COLOR);
            mActiveConnectingLineColor = mConnectingLineColor;
            mExpandedPinRadius = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    ta.getDimension(R.styleable.BubbleSelector_pinRadius,
            ...
            mPinPadding = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    ta.getDimension(R.styleable.BubbleSelector_pinPadding,
            ...
            mBarPaddingBottom = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                    ta.getDimension(R.styleable.BubbleSelector_barPaddingBottom,
                            DEFAULT_BAR_PADDING_BOTTOM_DP), getResources().getDisplayMetrics());
            mIsRangeBar = ta.getBoolean(R.styleable.BubbleSelector_rangeBar, true);
    ...
    private void releasePin(final PinView thumb) {
        ...
        if(Integer.valueOf(getPinValue(tickIndex)) == 1)
        {
            thumb.setXValue("Low");
        }
        else if((Integer.valueOf(getPinValue(tickIndex)) == 2))
        {
            thumb.setXValue("Medium");
        }
        else if((Integer.valueOf(getPinValue(tickIndex)) == 3))
        {
            thumb.setXValue("High");
        }
    ...
    public static interface OnRangeBarChangeListener {
        ...
        public void onRangeChangeListener(BubbleSelector rangeBar, int leftPinIndex,
